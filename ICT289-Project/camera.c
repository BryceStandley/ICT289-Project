...
//yaw is rotation on the x
//pitch is rotation on the y

void UpdateCameraLookAt(Camera* c)
{
	Vector3 v;
	//v.x = c->transform.Position.x + c->Forward.x;
	//v.y = c->transform.Position.y;
	//v.z = c->transform.Position.z + c->Forward.z;
